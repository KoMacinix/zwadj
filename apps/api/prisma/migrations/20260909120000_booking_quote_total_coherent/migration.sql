...
--
-- ⛔ ET CE N'EST PAS UNE IGNORANCE DE LA FORME : ELLE ÉTAIT À LA LIGNE D'À CÔTÉ.
-- Le même fichier `20260707000001` écrit DEUX identités arithmétiques en CHECK,
-- à une vingtaine de lignes des contraintes trouées — et, relevé, dans DEUX
-- contraintes de la MÊME famille de noms :
--     l. 203, `booking_services_amounts_valid` :
--         line_total_cents = unit_price_cents * quantity
--     l. 228, `commissions_amounts_valid` :
--         net_amount_cents = gross_amount_cents - incentive_deduction_cents
-- Sur les cinq `*_amounts_valid` du dépôt, celles qui avaient une identité à
-- écrire l'ont écrite ; les deux qui snapshotent un AGRÉGAT — `bookings` et
-- `quotes` — ne l'ont pas.
-- ⇒ Ce n'est donc pas « une contrainte a laissé passer » : c'est une INCOHÉRENCE
-- INTERNE À UN SEUL FICHIER, et c'est ce qui interdit de traiter les deux
-- contraintes ci-dessous de redondance.
--
-- -----------------------------------------------------------------------------
