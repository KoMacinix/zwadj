"""D305 — COPIE de `docs/preuves/D304/controle-forme-chargily.py` (pièce de D304, non retouchée), élargie à
ce que CE lot touche : les trois `.md` d'autorité, tout `docs/preuves/D305/`, et les fichiers de code et de harnais
du diff. Contrôle de FORME Chargily, À PART de l'audit (qui en est aveugle : décision 5 du relecteur, D304).
Depuis la racine :

    python3 docs/preuves/D305/controles/controle-forme-chargily.py
...
cibles += ["apps/api/src/venues/booking-locks.prisma.ts", "apps/api/src/venues/booking-transitions.ts",
           "apps/api/src/venues/booking-transitions.spec.ts", "apps/api/src/venues/bookings.service.ts",
           "apps/api/test/int/bookings.int-spec.ts", "neutralisation/neutralize-rang23.py",
           "neutralisation/neutralize-solid-s5b.py"]
for racine, _, noms in os.walk("docs/preuves/D305"):
